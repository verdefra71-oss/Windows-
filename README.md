...
L'icona dell'app Android/iOS è impostata sul logo **Il Tornitore di Emanuele Verde** presente in `assets/app_icon.png`.
Il workflow esegue automaticamente `dart run flutter_launcher_icons` prima della compilazione, quindi l'icona viene applicata all'APK e alla build iOS.


## Windows

Il progetto è predisposto anche per Windows. La compilazione viene eseguita da GitHub Actions su `windows-latest`, con Visual Studio disponibile nel runner.

- Database SQLite su Windows tramite `sqflite_common_ffi`.
- Icona Windows generata da `assets/app_icon.png`.
- Le notifiche programmate restano attive su Android; su Windows vengono disattivate senza impedire il funzionamento dell'app.
- L'artefatto Windows viene prodotto come `Gestione-Preventivi-Il-Tornitore-Windows.zip`.
