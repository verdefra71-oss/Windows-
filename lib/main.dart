...
void main() async {
  ...
  // SQLite su Windows usa il backend FFI.
  // Su Android/iOS resta attivo il backend nativo di sqflite.
...
class NotificationService {
  ...
  Future<void> init() async {
    // flutter_local_notifications non viene usato su Windows.
    // Le notifiche delle rate restano disponibili su Android.
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  int rate = 0;
  int rateDaPagare = 0;
  double totalePreventivi = 0;
  List<Map<String, dynamic>> ultimiPreventivi = [];
  bool loading = true;
  ...
  Future<void> caricaStatistiche() async {
    ...
    final preventiviRows = results[0];
    final rateRows = results[3];
    final totale = preventiviRows.fold<double>(
      0,
      (sum, row) => sum + ((row['totale'] as num?)?.toDouble() ?? 0),
    );
    final nonPagate = rateRows.where((r) => (r['pagata'] ?? 0) == 0).length;

    setState(() {
      preventivi = preventiviRows.length;
      clienti = results[1].length;
      prodotti = results[2].length;
      rate = rateRows.length;
      rateDaPagare = nonPagate;
      totalePreventivi = totale;
      ultimiPreventivi = preventiviRows.take(5).toList();
      loading = false;
    });
  }
        ...
        title: Row(
          children: [
            ...
            const Column(
              ...
              children: [
                Text('Gestione Preventivi', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
                ...
                child: ConstrainedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _sectionTitle('Panoramica', 'La tua attività in un colpo d’occhio'),
                      ...
                      GridView.count(
                        ...
                        children: [
                          _statCard(Icons.receipt_long_rounded, 'Preventivi', preventivi.toString(), scheme.primary),
                          _statCard(Icons.people_alt_rounded, 'Clienti', clienti.toString(), const Color(0xFF0F766E)),
                          _statCard(Icons.inventory_2_rounded, 'Prodotti / Servizi', prodotti.toString(), const Color(0xFF7C3AED)),
                          _statCard(Icons.payments_rounded, 'Da incassare', rateDaPagare.toString(), const Color(0xFFB45309)),
                        ],
                      ),
                      const SizedBox(height: 22),
                      if (wide)
                        Row(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Expanded(flex: 3, child: _recentCard(scheme)),
                            const SizedBox(width: 16),
                            Expanded(flex: 2, child: _quickActions(scheme)),
                          ],
                        )
                      else ...[
                        _recentCard(scheme),
  ...
  Widget _hero(ColorScheme scheme, bool wide) {
    return Container(
      width: double.infinity,
      padding: EdgeInsets.all(wide ? 28 : 20),
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [scheme.primary, Color.lerp(scheme.primary, Colors.black, .22)!],
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
        ),
        borderRadius: BorderRadius.circular(24),
        boxShadow: [BoxShadow(color: scheme.primary.withOpacity(.18), blurRadius: 24, offset: const Offset(0, 10))],
      ),
      child: wide
          ? Row(children: [Expanded(child: _heroText(scheme)), const SizedBox(width: 30), _heroLogo()])
          : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_heroText(scheme), const SizedBox(height: 18), _heroButton(scheme)]),
    );
  }

  Widget _heroText(ColorScheme scheme) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Bentornato!', style: TextStyle(color: Colors.white70, fontSize: 14, fontWeight: FontWeight.w600)),
        const SizedBox(height: 5),
        const Text('Crea il tuo prossimo preventivo.', style: TextStyle(color: Colors.white, fontSize: 26, fontWeight: FontWeight.w800)),
        const SizedBox(height: 8),
        const Text('Clienti, servizi, rate e PDF sempre sotto controllo.', style: TextStyle(color: Colors.white70, fontSize: 14)),
        const SizedBox(height: 18),
        _heroButton(scheme),
      ],
    );
  ...
  Widget _heroButton(ColorScheme scheme) {
    return FilledButton.icon(
      ...
      label: const Text('NUOVO PREVENTIVO', style: TextStyle(fontWeight: FontWeight.w800)),
    );
  }
  ...
  Widget _heroLogo() {
    return ClipRRect(
      ...
      child: Container(
        ...
        child: Image.asset('assets/logo.png', fit: BoxFit.contain, filterQuality: FilterQuality.high),
      ),
    );
  }
      ...
      child: Padding(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(label, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.grey, fontSize: 12)),
          ])),
        ]),
      ),
    );
  ...
  Widget _recentCard(ColorScheme scheme) {
    return Card(
      elevation: 0,
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            const Expanded(child: Text('Ultimi preventivi', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800))),
            TextButton(onPressed: () => apri(const ListaPreventiviScreen()), child: const Text('Vedi tutti')),
          ]),
          const SizedBox(height: 8),
          if (loading)
            const Padding(padding: EdgeInsets.all(20), child: Center(child: CircularProgressIndicator()))
          else if (ultimiPreventivi.isEmpty)
            _emptyRecent(scheme)
          else
            ...ultimiPreventivi.map((row) {
              final totale = (row['totale'] as num?)?.toDouble() ?? 0;
              return ListTile(
                ...
                title: Text((row['numero'] ?? 'Preventivo').toString(), style: const TextStyle(fontWeight: FontWeight.w700)),
                subtitle: Text('${row['cliente'] ?? ''}  •  ${_data(row['data']?.toString())}'),
                trailing: Text(_euro(totale), style: const TextStyle(fontWeight: FontWeight.w800)),
              );
            }),
        ]),
      ),
  ...
  Widget _emptyRecent(ColorScheme scheme) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 28),
      child: Center(child: Column(children: [
        Icon(Icons.receipt_long_outlined, size: 46, color: scheme.primary.withOpacity(.35)),
        const SizedBox(height: 10),
        const Text('Nessun preventivo ancora', style: TextStyle(fontWeight: FontWeight.w700)),
        const SizedBox(height: 4),
        const Text('Crea il primo preventivo per iniziare.', style: TextStyle(color: Colors.grey)),
      ])),
    );
  }
  ...
  Widget _quickActions(ColorScheme scheme) {
    final actions = [
      (Icons.people_alt_rounded, 'Clienti', 'Anagrafica clienti', const ClientiScreen()),
      (Icons.inventory_2_rounded, 'Prodotti / Servizi', 'Listino e prezzi', const ProdottiScreen()),
      (Icons.payments_rounded, 'Rate e scadenze', 'Controlla i pagamenti', const RateScreen()),
      (Icons.backup_rounded, 'Backup e dati', 'Salva o ripristina', const BackupScreen()),
      (Icons.notifications_active_rounded, 'Notifiche', 'Gestisci gli avvisi', const NotificheScreen()),
    ];
    return Card(
      elevation: 0,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
      child: Padding(
        padding: const EdgeInsets.all(18),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Accesso rapido', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
          const SizedBox(height: 8),
          ...actions.map((a) => _actionTile(scheme, a.$1, a.$2, a.$3, () => apri(a.$4))),
        ]),
      ),
    );
  }

  Widget _actionTile(ColorScheme scheme, IconData icon, String title, String subtitle, VoidCallback onTap) {
    return ListTile(
      contentPadding: const EdgeInsets.symmetric(horizontal: 0, vertical: 2),
      leading: Container(
        padding: const EdgeInsets.all(9),
        decoration: BoxDecoration(color: scheme.primary.withOpacity(.08), borderRadius: BorderRadius.circular(11)),
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Valore complessivo preventivi', style: TextStyle(fontWeight: FontWeight.w700)),
          Text('Totale dei preventivi presenti in archivio', style: TextStyle(color: Colors.grey, fontSize: 12)),
        ])),
        Text(loading ? '…' : _euro(totalePreventivi), style: TextStyle(color: scheme.primary, fontSize: 20, fontWeight: FontWeight.w900)),
      ]),
    );
  }
}
